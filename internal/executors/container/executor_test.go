...
func TestContainerCreateBasic(t *testing.T) {
	...
	tests := []struct {
		...
		{
			isSupport: true,
			name:      "container-support-create-basic",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, runningCmd, executor.GetRunningCommand(test.isSupport))
			assert.Nil(t, executor.GetPreviousRunningCommand(test.isSupport))
			assert.NoError(t, err)
			if err != nil {
